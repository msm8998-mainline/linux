// SPDX-License-Identifier: GPL-2.0-only
/* Copyright (c) 2025 Giovanni Ricca <giovanniricca@proton.me>
 * Generated with linux-mdss-dsi-panel-driver-generator from vendor device tree:
 * Copyright (c) 2025, The Linux Foundation. All rights reserved.
 */

#include <linux/delay.h>
#include <linux/gpio/consumer.h>
#include <linux/module.h>
#include <linux/of.h>

#include <video/mipi_display.h>

#include <drm/drm_mipi_dsi.h>
#include <drm/drm_modes.h>
#include <drm/drm_panel.h>

struct jdi_nt35596s {
	struct drm_panel panel;
	struct mipi_dsi_device *dsi;
	struct gpio_desc *reset_gpio;
	bool prepared;
};

static inline
struct jdi_nt35596s *to_jdi_nt35596s(struct drm_panel *panel)
{
	return container_of(panel, struct jdi_nt35596s, panel);
}

#define dsi_dcs_write_seq(dsi, seq...) do {				\
		static const u8 d[] = { seq };				\
		int ret;						\
		ret = mipi_dsi_dcs_write_buffer(dsi, d, ARRAY_SIZE(d));	\
		if (ret < 0)						\
			return ret;					\
	} while (0)

static void jdi_nt35596s_reset(struct jdi_nt35596s *ctx)
{
	gpiod_set_value_cansleep(ctx->reset_gpio, 0);
	usleep_range(5000, 6000);
	gpiod_set_value_cansleep(ctx->reset_gpio, 1);
	usleep_range(10000, 11000);
	gpiod_set_value_cansleep(ctx->reset_gpio, 0);
	usleep_range(10000, 11000);
}

static int jdi_nt35596s_on(struct jdi_nt35596s *ctx)
{
	struct mipi_dsi_device *dsi = ctx->dsi;
	struct device *dev = &dsi->dev;
	int ret;

	dsi->mode_flags |= MIPI_DSI_MODE_LPM;

	dsi_dcs_write_seq(dsi, 0xff, 0x20);
	dsi_dcs_write_seq(dsi, 0xfb, 0x01);
	dsi_dcs_write_seq(dsi, 0x75, 0x01);
	dsi_dcs_write_seq(dsi, 0x76, 0x2a);
	dsi_dcs_write_seq(dsi, 0x77, 0x01);
	dsi_dcs_write_seq(dsi, 0x78, 0x2f);
	dsi_dcs_write_seq(dsi, 0x79, 0x01);
	dsi_dcs_write_seq(dsi, 0x7a, 0x3a);
	dsi_dcs_write_seq(dsi, 0x7b, 0x01);
	dsi_dcs_write_seq(dsi, 0x7c, 0x44);
	dsi_dcs_write_seq(dsi, 0x7d, 0x01);
	dsi_dcs_write_seq(dsi, 0x7e, 0x4d);
	dsi_dcs_write_seq(dsi, 0x7f, 0x01);
	dsi_dcs_write_seq(dsi, 0x80, 0x56);
	dsi_dcs_write_seq(dsi, 0x81, 0x01);
	dsi_dcs_write_seq(dsi, 0x82, 0x5d);
	dsi_dcs_write_seq(dsi, 0x83, 0x01);
	dsi_dcs_write_seq(dsi, 0x84, 0x66);
	dsi_dcs_write_seq(dsi, 0x85, 0x01);
	dsi_dcs_write_seq(dsi, 0x86, 0x6e);
	dsi_dcs_write_seq(dsi, 0x87, 0x01);
	dsi_dcs_write_seq(dsi, 0x88, 0x88);
	dsi_dcs_write_seq(dsi, 0x89, 0x01);
	dsi_dcs_write_seq(dsi, 0x8a, 0xa1);
	dsi_dcs_write_seq(dsi, 0x8b, 0x01);
	dsi_dcs_write_seq(dsi, 0x8c, 0xc9);
	dsi_dcs_write_seq(dsi, 0x8d, 0x01);
	dsi_dcs_write_seq(dsi, 0x8e, 0xec);
	dsi_dcs_write_seq(dsi, 0x8f, 0x02);
	dsi_dcs_write_seq(dsi, 0x90, 0x25);
	dsi_dcs_write_seq(dsi, 0x91, 0x02);
	dsi_dcs_write_seq(dsi, 0x92, 0x53);
	dsi_dcs_write_seq(dsi, 0x93, 0x02);
	dsi_dcs_write_seq(dsi, 0x94, 0x55);
	dsi_dcs_write_seq(dsi, 0x95, 0x02);
	dsi_dcs_write_seq(dsi, 0x96, 0x86);
	dsi_dcs_write_seq(dsi, 0x97, 0x02);
	dsi_dcs_write_seq(dsi, 0x98, 0xbf);
	dsi_dcs_write_seq(dsi, 0x99, 0x02);
	dsi_dcs_write_seq(dsi, 0x9a, 0xe5);
	dsi_dcs_write_seq(dsi, 0x9b, 0x03);
	dsi_dcs_write_seq(dsi, 0x9c, 0x16);
	dsi_dcs_write_seq(dsi, 0x9d, 0x03);
	dsi_dcs_write_seq(dsi, 0x9e, 0x34);
	dsi_dcs_write_seq(dsi, 0x9f, 0x03);
	dsi_dcs_write_seq(dsi, 0xa0, 0x5b);
	dsi_dcs_write_seq(dsi, MIPI_DCS_READ_PPS_START, 0x03);
	dsi_dcs_write_seq(dsi, 0xa3, 0x68);
	dsi_dcs_write_seq(dsi, 0xa4, 0x03);
	dsi_dcs_write_seq(dsi, 0xa5, 0x75);
	dsi_dcs_write_seq(dsi, 0xa6, 0x03);
	dsi_dcs_write_seq(dsi, 0xa7, 0x85);
	dsi_dcs_write_seq(dsi, MIPI_DCS_READ_PPS_CONTINUE, 0x03);
	dsi_dcs_write_seq(dsi, 0xaa, 0x97);
	dsi_dcs_write_seq(dsi, 0xab, 0x03);
	dsi_dcs_write_seq(dsi, 0xac, 0xac);
	dsi_dcs_write_seq(dsi, 0xad, 0x03);
	dsi_dcs_write_seq(dsi, 0xae, 0xc3);
	dsi_dcs_write_seq(dsi, 0xaf, 0x03);
	dsi_dcs_write_seq(dsi, 0xb0, 0xce);
	dsi_dcs_write_seq(dsi, 0xb1, 0x03);
	dsi_dcs_write_seq(dsi, 0xb2, 0xff);
	dsi_dcs_write_seq(dsi, 0xb3, 0x01);
	dsi_dcs_write_seq(dsi, 0xb4, 0x2a);
	dsi_dcs_write_seq(dsi, 0xb5, 0x01);
	dsi_dcs_write_seq(dsi, 0xb6, 0x2f);
	dsi_dcs_write_seq(dsi, 0xb7, 0x01);
	dsi_dcs_write_seq(dsi, 0xb8, 0x3a);
	dsi_dcs_write_seq(dsi, 0xb9, 0x01);
	dsi_dcs_write_seq(dsi, 0xba, 0x44);
	dsi_dcs_write_seq(dsi, 0xbb, 0x01);
	dsi_dcs_write_seq(dsi, 0xbc, 0x4d);
	dsi_dcs_write_seq(dsi, 0xbd, 0x01);
	dsi_dcs_write_seq(dsi, 0xbe, 0x56);
	dsi_dcs_write_seq(dsi, 0xbf, 0x01);
	dsi_dcs_write_seq(dsi, 0xc0, 0x5d);
	dsi_dcs_write_seq(dsi, 0xc1, 0x01);
	dsi_dcs_write_seq(dsi, 0xc2, 0x66);
	dsi_dcs_write_seq(dsi, 0xc3, 0x01);
	dsi_dcs_write_seq(dsi, 0xc4, 0x6e);
	dsi_dcs_write_seq(dsi, 0xc5, 0x01);
	dsi_dcs_write_seq(dsi, 0xc6, 0x88);
	dsi_dcs_write_seq(dsi, 0xc7, 0x01);
	dsi_dcs_write_seq(dsi, 0xc8, 0xa1);
	dsi_dcs_write_seq(dsi, 0xc9, 0x01);
	dsi_dcs_write_seq(dsi, 0xca, 0xc9);
	dsi_dcs_write_seq(dsi, 0xcb, 0x01);
	dsi_dcs_write_seq(dsi, 0xcc, 0xec);
	dsi_dcs_write_seq(dsi, 0xcd, 0x02);
	dsi_dcs_write_seq(dsi, 0xce, 0x25);
	dsi_dcs_write_seq(dsi, 0xcf, 0x02);
	dsi_dcs_write_seq(dsi, 0xd0, 0x53);
	dsi_dcs_write_seq(dsi, 0xd1, 0x02);
	dsi_dcs_write_seq(dsi, 0xd2, 0x55);
	dsi_dcs_write_seq(dsi, 0xd3, 0x02);
	dsi_dcs_write_seq(dsi, 0xd4, 0x86);
	dsi_dcs_write_seq(dsi, 0xd5, 0x02);
	dsi_dcs_write_seq(dsi, 0xd6, 0xbf);
	dsi_dcs_write_seq(dsi, 0xd7, 0x02);
	dsi_dcs_write_seq(dsi, 0xd8, 0xe5);
	dsi_dcs_write_seq(dsi, 0xd9, 0x03);
	dsi_dcs_write_seq(dsi, 0xda, 0x16);
	dsi_dcs_write_seq(dsi, 0xdb, 0x03);
	dsi_dcs_write_seq(dsi, 0xdc, 0x34);
	dsi_dcs_write_seq(dsi, 0xdd, 0x03);
	dsi_dcs_write_seq(dsi, 0xde, 0x5b);
	dsi_dcs_write_seq(dsi, 0xdf, 0x03);
	dsi_dcs_write_seq(dsi, 0xe0, 0x68);
	dsi_dcs_write_seq(dsi, 0xe1, 0x03);
	dsi_dcs_write_seq(dsi, 0xe2, 0x75);
	dsi_dcs_write_seq(dsi, 0xe3, 0x03);
	dsi_dcs_write_seq(dsi, 0xe4, 0x85);
	dsi_dcs_write_seq(dsi, 0xe5, 0x03);
	dsi_dcs_write_seq(dsi, 0xe6, 0x97);
	dsi_dcs_write_seq(dsi, 0xe7, 0x03);
	dsi_dcs_write_seq(dsi, 0xe8, 0xac);
	dsi_dcs_write_seq(dsi, 0xe9, 0x03);
	dsi_dcs_write_seq(dsi, 0xea, 0xc3);
	dsi_dcs_write_seq(dsi, 0xeb, 0x03);
	dsi_dcs_write_seq(dsi, 0xec, 0xce);
	dsi_dcs_write_seq(dsi, 0xed, 0x03);
	dsi_dcs_write_seq(dsi, 0xee, 0xff);
	dsi_dcs_write_seq(dsi, 0xef, 0x00);
	dsi_dcs_write_seq(dsi, 0xf0, 0xb1);
	dsi_dcs_write_seq(dsi, 0xf1, 0x00);
	dsi_dcs_write_seq(dsi, 0xf2, 0xbe);
	dsi_dcs_write_seq(dsi, 0xf3, 0x00);
	dsi_dcs_write_seq(dsi, 0xf4, 0xd5);
	dsi_dcs_write_seq(dsi, 0xf5, 0x00);
	dsi_dcs_write_seq(dsi, 0xf6, 0xe9);
	dsi_dcs_write_seq(dsi, 0xf7, 0x00);
	dsi_dcs_write_seq(dsi, 0xf8, 0xfb);
	dsi_dcs_write_seq(dsi, 0xf9, 0x01);
	dsi_dcs_write_seq(dsi, 0xfa, 0x0b);
	dsi_dcs_write_seq(dsi, 0xff, 0x21);
	dsi_dcs_write_seq(dsi, 0xfb, 0x01);
	dsi_dcs_write_seq(dsi, 0x00, 0x01);
	dsi_dcs_write_seq(dsi, 0x01, 0x1a);
	dsi_dcs_write_seq(dsi, 0x02, 0x01);
	dsi_dcs_write_seq(dsi, 0x03, 0x27);
	dsi_dcs_write_seq(dsi, 0x04, 0x01);
	dsi_dcs_write_seq(dsi, 0x05, 0x33);
	dsi_dcs_write_seq(dsi, 0x06, 0x01);
	dsi_dcs_write_seq(dsi, 0x07, 0x5d);
	dsi_dcs_write_seq(dsi, 0x08, 0x01);
	dsi_dcs_write_seq(dsi, 0x09, 0x7e);
	dsi_dcs_write_seq(dsi, 0x0a, 0x01);
	dsi_dcs_write_seq(dsi, 0x0b, 0xb2);
	dsi_dcs_write_seq(dsi, 0x0c, 0x01);
	dsi_dcs_write_seq(dsi, 0x0d, 0xda);
	dsi_dcs_write_seq(dsi, 0x0e, 0x02);
	dsi_dcs_write_seq(dsi, 0x0f, 0x1b);
	dsi_dcs_write_seq(dsi, 0x10, 0x02);
	dsi_dcs_write_seq(dsi, 0x11, 0x4d);
	dsi_dcs_write_seq(dsi, 0x12, 0x02);
	dsi_dcs_write_seq(dsi, 0x13, 0x4f);
	dsi_dcs_write_seq(dsi, 0x14, 0x02);
	dsi_dcs_write_seq(dsi, 0x15, 0x81);
	dsi_dcs_write_seq(dsi, 0x16, 0x02);
	dsi_dcs_write_seq(dsi, 0x17, 0xb9);
	dsi_dcs_write_seq(dsi, 0x18, 0x02);
	dsi_dcs_write_seq(dsi, 0x19, 0xdf);
	dsi_dcs_write_seq(dsi, 0x1a, 0x03);
	dsi_dcs_write_seq(dsi, 0x1b, 0x11);
	dsi_dcs_write_seq(dsi, 0x1c, 0x03);
	dsi_dcs_write_seq(dsi, 0x1d, 0x2f);
	dsi_dcs_write_seq(dsi, 0x1e, 0x03);
	dsi_dcs_write_seq(dsi, 0x1f, 0x56);
	dsi_dcs_write_seq(dsi, 0x20, 0x03);
	dsi_dcs_write_seq(dsi, 0x21, 0x62);
	dsi_dcs_write_seq(dsi, 0x22, 0x03);
	dsi_dcs_write_seq(dsi, 0x23, 0x6f);
	dsi_dcs_write_seq(dsi, 0x24, 0x03);
	dsi_dcs_write_seq(dsi, 0x25, 0x7e);
	dsi_dcs_write_seq(dsi, MIPI_DCS_SET_GAMMA_CURVE, 0x03);
	dsi_dcs_write_seq(dsi, 0x27, 0x90);
	dsi_dcs_write_seq(dsi, 0x28, 0x03);
	dsi_dcs_write_seq(dsi, 0x29, 0xa4);
	dsi_dcs_write_seq(dsi, 0x2a, 0x03);
	dsi_dcs_write_seq(dsi, 0x2b, 0xbb);
	dsi_dcs_write_seq(dsi, MIPI_DCS_WRITE_LUT, 0x03);
	dsi_dcs_write_seq(dsi, 0x2f, 0xca);
	dsi_dcs_write_seq(dsi, MIPI_DCS_SET_PARTIAL_ROWS, 0x03);
	dsi_dcs_write_seq(dsi, MIPI_DCS_SET_PARTIAL_COLUMNS, 0xff);
	dsi_dcs_write_seq(dsi, 0x32, 0x00);
	dsi_dcs_write_seq(dsi, 0x33, 0xb1);
	dsi_dcs_write_seq(dsi, 0x34, 0x00);
	dsi_dcs_write_seq(dsi, 0x35, 0xbe);
	dsi_dcs_write_seq(dsi, MIPI_DCS_SET_ADDRESS_MODE, 0x00);
	dsi_dcs_write_seq(dsi, 0x37, 0xd5);
	dsi_dcs_write_seq(dsi, 0x38, 0x00);
	dsi_dcs_write_seq(dsi, 0x39, 0xe9);

	ret = mipi_dsi_dcs_set_pixel_format(dsi, 0x00);
	if (ret < 0) {
		dev_err(dev, "Failed to set pixel format: %d\n", ret);
		return ret;
	}

	dsi_dcs_write_seq(dsi, 0x3b, 0xfb);
	dsi_dcs_write_seq(dsi, MIPI_DCS_SET_3D_CONTROL, 0x01);
	dsi_dcs_write_seq(dsi, 0x3f, 0x0b);
	dsi_dcs_write_seq(dsi, MIPI_DCS_SET_VSYNC_TIMING, 0x01);
	dsi_dcs_write_seq(dsi, 0x41, 0x1a);
	dsi_dcs_write_seq(dsi, 0x42, 0x01);
	dsi_dcs_write_seq(dsi, 0x43, 0x27);
	dsi_dcs_write_seq(dsi, 0x44, 0x01);
	dsi_dcs_write_seq(dsi, MIPI_DCS_GET_SCANLINE, 0x33);
	dsi_dcs_write_seq(dsi, 0x46, 0x01);
	dsi_dcs_write_seq(dsi, 0x47, 0x5d);
	dsi_dcs_write_seq(dsi, 0x48, 0x01);
	dsi_dcs_write_seq(dsi, 0x49, 0x7e);
	dsi_dcs_write_seq(dsi, 0x4a, 0x01);
	dsi_dcs_write_seq(dsi, 0x4b, 0xb2);
	dsi_dcs_write_seq(dsi, 0x4c, 0x01);
	dsi_dcs_write_seq(dsi, 0x4d, 0xda);
	dsi_dcs_write_seq(dsi, 0x4e, 0x02);
	dsi_dcs_write_seq(dsi, 0x4f, 0x1b);
	dsi_dcs_write_seq(dsi, 0x50, 0x02);

	ret = mipi_dsi_dcs_set_display_brightness(dsi, 0x004d);
	if (ret < 0) {
		dev_err(dev, "Failed to set display brightness: %d\n", ret);
		return ret;
	}

	dsi_dcs_write_seq(dsi, 0x52, 0x02);
	dsi_dcs_write_seq(dsi, MIPI_DCS_WRITE_CONTROL_DISPLAY, 0x4f);
	dsi_dcs_write_seq(dsi, 0x54, 0x02);
	dsi_dcs_write_seq(dsi, MIPI_DCS_WRITE_POWER_SAVE, 0x81);
	dsi_dcs_write_seq(dsi, 0x56, 0x02);
	dsi_dcs_write_seq(dsi, 0x58, 0xb9);
	dsi_dcs_write_seq(dsi, 0x59, 0x02);
	dsi_dcs_write_seq(dsi, 0x5a, 0xdf);
	dsi_dcs_write_seq(dsi, 0x5b, 0x03);
	dsi_dcs_write_seq(dsi, 0x5c, 0x11);
	dsi_dcs_write_seq(dsi, 0x5d, 0x03);
	dsi_dcs_write_seq(dsi, MIPI_DCS_SET_CABC_MIN_BRIGHTNESS, 0x2f);
	dsi_dcs_write_seq(dsi, 0x5f, 0x03);
	dsi_dcs_write_seq(dsi, 0x60, 0x56);
	dsi_dcs_write_seq(dsi, 0x61, 0x03);
	dsi_dcs_write_seq(dsi, 0x62, 0x62);
	dsi_dcs_write_seq(dsi, 0x63, 0x03);
	dsi_dcs_write_seq(dsi, 0x64, 0x6f);
	dsi_dcs_write_seq(dsi, 0x65, 0x03);
	dsi_dcs_write_seq(dsi, 0x66, 0x7e);
	dsi_dcs_write_seq(dsi, 0x67, 0x03);
	dsi_dcs_write_seq(dsi, 0x68, 0x90);
	dsi_dcs_write_seq(dsi, 0x69, 0x03);
	dsi_dcs_write_seq(dsi, 0x6a, 0xa4);
	dsi_dcs_write_seq(dsi, 0x6b, 0x03);
	dsi_dcs_write_seq(dsi, 0x6c, 0xbb);
	dsi_dcs_write_seq(dsi, 0x6d, 0x03);
	dsi_dcs_write_seq(dsi, 0x6e, 0xca);
	dsi_dcs_write_seq(dsi, 0x6f, 0x03);
	dsi_dcs_write_seq(dsi, 0x70, 0xff);
	dsi_dcs_write_seq(dsi, 0x71, 0x00);
	dsi_dcs_write_seq(dsi, 0x72, 0x00);
	dsi_dcs_write_seq(dsi, 0x73, 0x00);
	dsi_dcs_write_seq(dsi, 0x74, 0x38);
	dsi_dcs_write_seq(dsi, 0x75, 0x00);
	dsi_dcs_write_seq(dsi, 0x76, 0x68);
	dsi_dcs_write_seq(dsi, 0x77, 0x00);
	dsi_dcs_write_seq(dsi, 0x78, 0x90);
	dsi_dcs_write_seq(dsi, 0x79, 0x00);
	dsi_dcs_write_seq(dsi, 0x7a, 0xae);
	dsi_dcs_write_seq(dsi, 0x7b, 0x00);
	dsi_dcs_write_seq(dsi, 0x7c, 0xc7);
	dsi_dcs_write_seq(dsi, 0x7d, 0x00);
	dsi_dcs_write_seq(dsi, 0x7e, 0xdf);
	dsi_dcs_write_seq(dsi, 0x7f, 0x00);
	dsi_dcs_write_seq(dsi, 0x80, 0xf0);
	dsi_dcs_write_seq(dsi, 0x81, 0x01);
	dsi_dcs_write_seq(dsi, 0x82, 0x03);
	dsi_dcs_write_seq(dsi, 0x83, 0x01);
	dsi_dcs_write_seq(dsi, 0x84, 0x3a);
	dsi_dcs_write_seq(dsi, 0x85, 0x01);
	dsi_dcs_write_seq(dsi, 0x86, 0x63);
	dsi_dcs_write_seq(dsi, 0x87, 0x01);
	dsi_dcs_write_seq(dsi, 0x88, 0xa2);
	dsi_dcs_write_seq(dsi, 0x89, 0x01);
	dsi_dcs_write_seq(dsi, 0x8a, 0xcd);
	dsi_dcs_write_seq(dsi, 0x8b, 0x02);
	dsi_dcs_write_seq(dsi, 0x8c, 0x12);
	dsi_dcs_write_seq(dsi, 0x8d, 0x02);
	dsi_dcs_write_seq(dsi, 0x8e, 0x45);
	dsi_dcs_write_seq(dsi, 0x8f, 0x02);
	dsi_dcs_write_seq(dsi, 0x90, 0x47);
	dsi_dcs_write_seq(dsi, 0x91, 0x02);
	dsi_dcs_write_seq(dsi, 0x92, 0x7a);
	dsi_dcs_write_seq(dsi, 0x93, 0x02);
	dsi_dcs_write_seq(dsi, 0x94, 0xb4);
	dsi_dcs_write_seq(dsi, 0x95, 0x02);
	dsi_dcs_write_seq(dsi, 0x96, 0xda);
	dsi_dcs_write_seq(dsi, 0x97, 0x03);
	dsi_dcs_write_seq(dsi, 0x98, 0x0e);
	dsi_dcs_write_seq(dsi, 0x99, 0x03);
	dsi_dcs_write_seq(dsi, 0x9a, 0x2c);
	dsi_dcs_write_seq(dsi, 0x9b, 0x03);
	dsi_dcs_write_seq(dsi, 0x9c, 0x55);
	dsi_dcs_write_seq(dsi, 0x9d, 0x03);
	dsi_dcs_write_seq(dsi, 0x9e, 0x61);
	dsi_dcs_write_seq(dsi, 0x9f, 0x03);
	dsi_dcs_write_seq(dsi, 0xa0, 0x6f);
	dsi_dcs_write_seq(dsi, MIPI_DCS_READ_PPS_START, 0x03);
	dsi_dcs_write_seq(dsi, 0xa3, 0x7f);
	dsi_dcs_write_seq(dsi, 0xa4, 0x03);
	dsi_dcs_write_seq(dsi, 0xa5, 0x93);
	dsi_dcs_write_seq(dsi, 0xa6, 0x03);
	dsi_dcs_write_seq(dsi, 0xa7, 0xa5);
	dsi_dcs_write_seq(dsi, MIPI_DCS_READ_PPS_CONTINUE, 0x03);
	dsi_dcs_write_seq(dsi, 0xaa, 0xba);
	dsi_dcs_write_seq(dsi, 0xab, 0x03);
	dsi_dcs_write_seq(dsi, 0xac, 0xc9);
	dsi_dcs_write_seq(dsi, 0xad, 0x03);
	dsi_dcs_write_seq(dsi, 0xae, 0xff);
	dsi_dcs_write_seq(dsi, 0xaf, 0x00);
	dsi_dcs_write_seq(dsi, 0xb0, 0x00);
	dsi_dcs_write_seq(dsi, 0xb1, 0x00);
	dsi_dcs_write_seq(dsi, 0xb2, 0x38);
	dsi_dcs_write_seq(dsi, 0xb3, 0x00);
	dsi_dcs_write_seq(dsi, 0xb4, 0x68);
	dsi_dcs_write_seq(dsi, 0xb5, 0x00);
	dsi_dcs_write_seq(dsi, 0xb6, 0x90);
	dsi_dcs_write_seq(dsi, 0xb7, 0x00);
	dsi_dcs_write_seq(dsi, 0xb8, 0xae);
	dsi_dcs_write_seq(dsi, 0xb9, 0x00);
	dsi_dcs_write_seq(dsi, 0xba, 0xc7);
	dsi_dcs_write_seq(dsi, 0xbb, 0x00);
	dsi_dcs_write_seq(dsi, 0xbc, 0xdf);
	dsi_dcs_write_seq(dsi, 0xbd, 0x00);
	dsi_dcs_write_seq(dsi, 0xbe, 0xf0);
	dsi_dcs_write_seq(dsi, 0xbf, 0x01);
	dsi_dcs_write_seq(dsi, 0xc0, 0x03);
	dsi_dcs_write_seq(dsi, 0xc1, 0x01);
	dsi_dcs_write_seq(dsi, 0xc2, 0x3a);
	dsi_dcs_write_seq(dsi, 0xc3, 0x01);
	dsi_dcs_write_seq(dsi, 0xc4, 0x63);
	dsi_dcs_write_seq(dsi, 0xc5, 0x01);
	dsi_dcs_write_seq(dsi, 0xc6, 0xa2);
	dsi_dcs_write_seq(dsi, 0xc7, 0x01);
	dsi_dcs_write_seq(dsi, 0xc8, 0xcd);
	dsi_dcs_write_seq(dsi, 0xc9, 0x02);
	dsi_dcs_write_seq(dsi, 0xca, 0x12);
	dsi_dcs_write_seq(dsi, 0xcb, 0x02);
	dsi_dcs_write_seq(dsi, 0xcc, 0x45);
	dsi_dcs_write_seq(dsi, 0xcd, 0x02);
	dsi_dcs_write_seq(dsi, 0xce, 0x47);
	dsi_dcs_write_seq(dsi, 0xcf, 0x02);
	dsi_dcs_write_seq(dsi, 0xd0, 0x7a);
	dsi_dcs_write_seq(dsi, 0xd1, 0x02);
	dsi_dcs_write_seq(dsi, 0xd2, 0xb4);
	dsi_dcs_write_seq(dsi, 0xd3, 0x02);
	dsi_dcs_write_seq(dsi, 0xd4, 0xda);
	dsi_dcs_write_seq(dsi, 0xd5, 0x03);
	dsi_dcs_write_seq(dsi, 0xd6, 0x0e);
	dsi_dcs_write_seq(dsi, 0xd7, 0x03);
	dsi_dcs_write_seq(dsi, 0xd8, 0x2c);
	dsi_dcs_write_seq(dsi, 0xd9, 0x03);
	dsi_dcs_write_seq(dsi, 0xda, 0x55);
	dsi_dcs_write_seq(dsi, 0xdb, 0x03);
	dsi_dcs_write_seq(dsi, 0xdc, 0x61);
	dsi_dcs_write_seq(dsi, 0xdd, 0x03);
	dsi_dcs_write_seq(dsi, 0xde, 0x6f);
	dsi_dcs_write_seq(dsi, 0xdf, 0x03);
	dsi_dcs_write_seq(dsi, 0xe0, 0x7f);
	dsi_dcs_write_seq(dsi, 0xe1, 0x03);
	dsi_dcs_write_seq(dsi, 0xe2, 0x93);
	dsi_dcs_write_seq(dsi, 0xe3, 0x03);
	dsi_dcs_write_seq(dsi, 0xe4, 0xa5);
	dsi_dcs_write_seq(dsi, 0xe5, 0x03);
	dsi_dcs_write_seq(dsi, 0xe6, 0xba);
	dsi_dcs_write_seq(dsi, 0xe7, 0x03);
	dsi_dcs_write_seq(dsi, 0xe8, 0xc9);
	dsi_dcs_write_seq(dsi, 0xe9, 0x03);
	dsi_dcs_write_seq(dsi, 0xea, 0xff);
	dsi_dcs_write_seq(dsi, 0xff, 0x26);
	dsi_dcs_write_seq(dsi, 0xfb, 0x01);
	dsi_dcs_write_seq(dsi, 0x03, 0x18);
	dsi_dcs_write_seq(dsi, 0xff, 0xe0);
	dsi_dcs_write_seq(dsi, 0xfb, 0x01);
	dsi_dcs_write_seq(dsi, 0x24, 0xca);
	dsi_dcs_write_seq(dsi, 0xff, 0x23);
	dsi_dcs_write_seq(dsi, 0xfb, 0x01);
	dsi_dcs_write_seq(dsi, 0x00, 0x00);
	dsi_dcs_write_seq(dsi, 0x01, 0x84);
	dsi_dcs_write_seq(dsi, 0x05, 0x2d);
	dsi_dcs_write_seq(dsi, 0x06, 0x00);
	dsi_dcs_write_seq(dsi, 0xff, 0x27);
	dsi_dcs_write_seq(dsi, 0x1e, 0x55);
	dsi_dcs_write_seq(dsi, 0xfb, 0x01);
	dsi_dcs_write_seq(dsi, 0xff, 0x24);
	dsi_dcs_write_seq(dsi, 0x9d, 0x34);
	dsi_dcs_write_seq(dsi, 0xfb, 0x01);
	dsi_dcs_write_seq(dsi, 0xff, 0x20);
	dsi_dcs_write_seq(dsi, 0x1f, 0xff);
	dsi_dcs_write_seq(dsi, 0xfb, 0x01);
	dsi_dcs_write_seq(dsi, 0xff, 0x10);

	ret = mipi_dsi_dcs_set_tear_on(dsi, MIPI_DSI_DCS_TEAR_MODE_VHBLANK);
	if (ret < 0) {
		dev_err(dev, "Failed to set tear on: %d\n", ret);
		return ret;
	}

	ret = mipi_dsi_dcs_set_display_brightness(dsi, 0x00ff);
	if (ret < 0) {
		dev_err(dev, "Failed to set display brightness: %d\n", ret);
		return ret;
	}

	dsi_dcs_write_seq(dsi, MIPI_DCS_WRITE_CONTROL_DISPLAY, 0x24);
	dsi_dcs_write_seq(dsi, MIPI_DCS_WRITE_POWER_SAVE, 0x00);
	dsi_dcs_write_seq(dsi, 0xb0, 0x00);

	ret = mipi_dsi_dcs_exit_sleep_mode(dsi);
	if (ret < 0) {
		dev_err(dev, "Failed to exit sleep mode: %d\n", ret);
		return ret;
	}
	msleep(80);

	ret = mipi_dsi_dcs_set_display_on(dsi);
	if (ret < 0) {
		dev_err(dev, "Failed to set display on: %d\n", ret);
		return ret;
	}

	return 0;
}

static int jdi_nt35596s_off(struct jdi_nt35596s *ctx)
{
	struct mipi_dsi_device *dsi = ctx->dsi;
	struct device *dev = &dsi->dev;
	int ret;

	dsi->mode_flags &= ~MIPI_DSI_MODE_LPM;

	ret = mipi_dsi_dcs_set_display_off(dsi);
	if (ret < 0) {
		dev_err(dev, "Failed to set display off: %d\n", ret);
		return ret;
	}
	msleep(20);

	ret = mipi_dsi_dcs_enter_sleep_mode(dsi);
	if (ret < 0) {
		dev_err(dev, "Failed to enter sleep mode: %d\n", ret);
		return ret;
	}
	msleep(120);

	return 0;
}

static int jdi_nt35596s_prepare(struct drm_panel *panel)
{
	struct jdi_nt35596s *ctx = to_jdi_nt35596s(panel);
	struct device *dev = &ctx->dsi->dev;
	int ret;

	if (ctx->prepared)
		return 0;

	jdi_nt35596s_reset(ctx);

	ret = jdi_nt35596s_on(ctx);
	if (ret < 0) {
		dev_err(dev, "Failed to initialize panel: %d\n", ret);
		gpiod_set_value_cansleep(ctx->reset_gpio, 1);
		return ret;
	}

	ctx->prepared = true;
	return 0;
}

static int jdi_nt35596s_unprepare(struct drm_panel *panel)
{
	struct jdi_nt35596s *ctx = to_jdi_nt35596s(panel);
	struct device *dev = &ctx->dsi->dev;
	int ret;

	if (!ctx->prepared)
		return 0;

	ret = jdi_nt35596s_off(ctx);
	if (ret < 0)
		dev_err(dev, "Failed to un-initialize panel: %d\n", ret);

	gpiod_set_value_cansleep(ctx->reset_gpio, 1);

	ctx->prepared = false;
	return 0;
}

static const struct drm_display_mode jdi_nt35596s_mode = {
	.clock = (1080 + 12 + 4 + 64) * (2040 + 4 + 1 + 23) * 60 / 1000,
	.hdisplay = 1080,
	.hsync_start = 1080 + 12,
	.hsync_end = 1080 + 12 + 4,
	.htotal = 1080 + 12 + 4 + 64,
	.vdisplay = 2040,
	.vsync_start = 2040 + 4,
	.vsync_end = 2040 + 4 + 1,
	.vtotal = 2040 + 4 + 1 + 23,
	.width_mm = 68,
	.height_mm = 128,
};

static int jdi_nt35596s_get_modes(struct drm_panel *panel,
					    struct drm_connector *connector)
{
	struct drm_display_mode *mode;

	mode = drm_mode_duplicate(connector->dev, &jdi_nt35596s_mode);
	if (!mode)
		return -ENOMEM;

	drm_mode_set_name(mode);

	mode->type = DRM_MODE_TYPE_DRIVER | DRM_MODE_TYPE_PREFERRED;
	connector->display_info.width_mm = mode->width_mm;
	connector->display_info.height_mm = mode->height_mm;
	drm_mode_probed_add(connector, mode);

	return 1;
}

static const struct drm_panel_funcs jdi_nt35596s_panel_funcs = {
	.prepare = jdi_nt35596s_prepare,
	.unprepare = jdi_nt35596s_unprepare,
	.get_modes = jdi_nt35596s_get_modes,
};

static int jdi_nt35596s_probe(struct mipi_dsi_device *dsi)
{
	struct device *dev = &dsi->dev;
	struct jdi_nt35596s *ctx;
	int ret;

	ctx = devm_kzalloc(dev, sizeof(*ctx), GFP_KERNEL);
	if (!ctx)
		return -ENOMEM;

	ctx->reset_gpio = devm_gpiod_get(dev, "reset", GPIOD_OUT_HIGH);
	if (IS_ERR(ctx->reset_gpio))
		return dev_err_probe(dev, PTR_ERR(ctx->reset_gpio),
				     "Failed to get reset-gpios\n");

	ctx->dsi = dsi;
	mipi_dsi_set_drvdata(dsi, ctx);

	dsi->lanes = 4;
	dsi->format = MIPI_DSI_FMT_RGB888;
	dsi->mode_flags = MIPI_DSI_MODE_VIDEO | MIPI_DSI_MODE_NO_EOT_PACKET |
			  MIPI_DSI_CLOCK_NON_CONTINUOUS;

	drm_panel_init(&ctx->panel, dev, &jdi_nt35596s_panel_funcs,
		       DRM_MODE_CONNECTOR_DSI);

	ret = drm_panel_of_backlight(&ctx->panel);
	if (ret)
		return dev_err_probe(dev, ret, "Failed to get backlight\n");

	drm_panel_add(&ctx->panel);

	ret = mipi_dsi_attach(dsi);
	if (ret < 0) {
		dev_err(dev, "Failed to attach to DSI host: %d\n", ret);
		drm_panel_remove(&ctx->panel);
		return ret;
	}

	return 0;
}

static void jdi_nt35596s_remove(struct mipi_dsi_device *dsi)
{
	struct jdi_nt35596s *ctx = mipi_dsi_get_drvdata(dsi);
	int ret;

	ret = mipi_dsi_detach(dsi);
	if (ret < 0)
		dev_err(&dsi->dev, "Failed to detach from DSI host: %d\n", ret);

	drm_panel_remove(&ctx->panel);
}

static const struct of_device_id jdi_nt35596s_of_match[] = {
	{ .compatible = "jdi,fhd-nt35596s" },
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, jdi_nt35596s_of_match);

static struct mipi_dsi_driver jdi_nt35596s_driver = {
	.probe = jdi_nt35596s_probe,
	.remove = jdi_nt35596s_remove,
	.driver = {
		.name = "panel-jdi-nt35596s",
		.of_match_table = jdi_nt35596s_of_match,
	},
};
module_mipi_dsi_driver(jdi_nt35596s_driver);

MODULE_AUTHOR("Giovanni Ricca <giovanniricca@proton.me>");
MODULE_DESCRIPTION("DRM driver for jdi nt35596s 1080x2040 mipi video panel");
MODULE_LICENSE("GPL");
